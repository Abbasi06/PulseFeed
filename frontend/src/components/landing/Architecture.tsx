...
const ConnectionLine = ({ x1, y1, x2, y2, delay = 0 }: any) => (
    <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
        <motion.line
            ...
            transition={{ 
                strokeDashoffset: { duration: 10, repeat: Infinity, ease: "linear", delay },
                opacity: { duration: 1 }
            }}
        />
        <defs>
        ...
        <div className="relative w-full h-[500px] flex items-center justify-center">
            ...
            >
                <div className="w-full h-full rounded-full bg-[#010101] flex flex-col items-center justify-center border border-white/20">
                    ...
                    <span className="text-[10px] font-bold text-white uppercase tracking-tighter">The Swarm</span>
                </div>
            </motion.div>

            {/* Satellite Nodes */}
            <div className="absolute top-0 left-1/2 -translate-x-1/2">
                <Node title="Sourcing_Agent" desc="Scraping ArXiv, GitHub & Tech Blogs" icon={Globe} delay={0.2} />
                <ConnectionLine x1="50%" y1="100%" x2="50%" y2="200" delay={0.1} />
            </div>

            <div className="absolute bottom-0 left-1/2 -translate-x-1/2">
                <Node title="Delivery_Node" desc="Web & Mobile Cached Sync" icon={Layers} delay={0.8} />
                <ConnectionLine x1="50%" y1="-100%" x2="50%" y2="300" delay={0.7} />
            </div>

            <div className="absolute top-1/2 left-0 -translate-y-1/2">
                <Node title="Vector_DB" desc="High-Dimensional Embedding RAG" icon={Database} delay={0.4} />
                <ConnectionLine x1="100%" y1="50%" x2="400" y2="50%" delay={0.3} />
            </div>

            <div className="absolute top-1/2 right-0 -translate-y-1/2">
                <Node title="Synthesis_LLM" desc="Agentic Reasoners & Brief Gen" icon={Cpu} delay={0.6} />
                <ConnectionLine x1="-100%" y1="50%" x2="600" y2="50%" delay={0.5} />
            </div>

            {/* Central Pulsing Rings */}
            <motion.div animate={{ scale: [1, 2], opacity: [0.2, 0] }} transition={{ duration: 3, repeat: Infinity }} className="absolute z-0 w-32 h-32 rounded-full border border-[#B7397A]" />
            <motion.div animate={{ scale: [1, 2.5], opacity: [0.1, 0] }} transition={{ duration: 4, repeat: Infinity, delay: 1 }} className="absolute z-0 w-32 h-32 rounded-full border border-white/10" />
        </div>

        {/* Bottom Text Detail */}
        <div className="mt-28 text-center max-w-2xl mx-auto">
            <p className="text-white/40 text-sm leading-relaxed italic">
               "We focus on the orchestration of high-dimensional technical context. 
               By coordinating specialized agents for retrieval, ranking, and synthesis, 
               PulseFeed creates a persistent intelligence layer for the modern developer."
            </p>
        </div>
      </div>
    </section>
