...
import { UserPlus, Zap, Terminal, Laptop, Smartphone, Search, CheckCircle2, Navigation, Layers, Filter } from "lucide-react";

// Step 1 UI: Context Sync
const ContextSyncUI = () => (
    <div className="relative w-full h-full p-4 flex flex-col gap-3">
        <div className="flex items-center justify-between border-b border-white/5 pb-2">
            <span className="text-[9px] font-bold text-white/30 uppercase tracking-widest">Context_Sync.v1</span>
        ...
        <div className="flex flex-wrap gap-2">
            {["vLLM", "RAG", "KEDA", "Nvidia", "PyTorch"].map((t, i) => (
                <motion.div
                    key={i}
                    animate={{ 
                        backgroundColor: i < 3 ? "rgba(183, 57, 122, 0.2)" : "rgba(255, 255, 255, 0.05)",
                        borderColor: i < 3 ? "rgba(183, 57, 122, 0.4)" : "rgba(255, 255, 255, 0.1)"
                    }}
        ...
        <div className="mt-auto flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 border border-white/10">
            ...
            <div className="h-1 bg-white/10 flex-1 rounded-full overflow-hidden">
                <motion.div animate={{ x: [-50, 100] }} transition={{ duration: 2, repeat: Infinity }} className="h-full w-1/3 bg-[#B7397A]" />
            </div>
        </div>
    </div>
);
...
const QualityGateUI = () => (
    <div className="relative w-full h-full p-4 flex flex-col justify-center items-center">
        <div className="absolute top-4 left-4 text-[9px] font-mono text-white/30 uppercase">Noise_Reduction_Engine</div>
        <div className="relative w-32 h-32 flex items-center justify-center">
            ...
            <div className="relative z-10 flex flex-col items-center">
                ...
                <span className="text-[8px] text-white/40 uppercase font-bold">Noise Stripped</span>
            </div>
            {/* Pulsing rings */}
            <motion.div animate={{ scale: [1, 1.5, 1], opacity: [0.1, 0, 0.1] }} transition={{ duration: 2, repeat: Infinity }} className="absolute inset-0 rounded-full border border-[#B7397A]/20" />
        </div>
    </div>
);
...
const SynthesisUI = () => (
    <div className="relative w-full h-full p-4 flex flex-col gap-3">
        <div className="flex items-center gap-2 mb-1">
            <div className="px-2 py-0.5 rounded bg-[#4C6E94]/20 border border-[#4C6E94]/40 text-[8px] text-[#4C6E94] font-bold">SYNTHESIS_REPORT</div>
            <div className="h-px flex-1 bg-white/5" />
        </div>
        <div className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-2">
            <div className="flex justify-between items-start">
                <h5 className="text-[11px] font-bold text-white leading-tight">Quantization vs LoRA: A Performance Benchmark</h5>
        ...
        <div className="flex justify-between items-center mt-auto">
            <div className="flex -space-x-2">
                {[1, 2, 3].map(i => <div key={i} className="w-5 h-5 rounded-full bg-white/10 border border-black shadow-xl" />)}
            </div>
            <button className="px-3 py-1 rounded bg-[#B7397A] text-white text-[9px] font-bold hover:bg-[#B7397A]/80 transition-colors">Read Brief</button>
        </div>
    </div>
);
    ...
    <div className="relative w-full h-full flex flex-col items-center justify-center p-4">
        <div className="relative flex items-center gap-8">
            ...
            <div className="relative">
                ...
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 text-[8px] font-mono text-white/20">SYNC_PASS</div>
            </div>
            <div className="flex flex-col items-center gap-2">
...
const steps = [
  {
    ...
    description: "Define your technical context. Our multi-agent swarm maps your sub-interests across 100+ daily data sources.",
  ...
  },
  {
    ...
    description: "The 'Quality Gate' analyzes content for depth and technical merit—stripping away SEO bait and marketing fluff.",
  ...
  },
  {
    ...
    description: "Complex papers and repos are distilled into hyper-personalized briefs. We synthesize signal, not just links.",
    visual: SynthesisUI,
    tag: "RAG_Synthesizer"
  },
  {
    ...
    description: "Your technical context follows you. Pre-computed and cached for zero-latency access across all your devices.",
    visual: GlobalFlowUI,
    tag: "Edge_Sync"
  },
];
...
export default function HowItWorks() {
  return (
    <section id="process" className="relative w-full py-48 bg-[#010101] font-sans selection:bg-[#B7397A]/30 border-t border-white/5">
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-3">
                    <span className="text-[10px] font-mono text-[#B7397A] font-bold tracking-widest">{step.tag}</span>
