...
import { Database, Zap, Globe, FileText, Code, Search, ShieldCheck } from "lucide-react";

// Trace Line Component
const TraceLine = ({ rotate = 0 }: { rotate?: number }) => (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ transform: `rotate(${rotate}deg)` }}>
...
const LiveTerminal = () => (
    <div className="mt-8 w-full max-w-md rounded-xl bg-black/60 border border-white/5 p-4 font-mono text-[10px] shadow-2xl overflow-hidden h-32 relative">
        <div className="flex gap-1.5 mb-3 border-b border-white/5 pb-2">
            ...
            <span className="ml-2 text-white/20 uppercase tracking-[2px]">Swarm_Monitor.sh</span>
        </div>
        <div className="space-y-1 text-white/40">
            <motion.p animate={{ opacity: [0, 1] }} transition={{ delay: 0.2 }}>[09:44:01] <span className="text-[#B7397A]">SWARM_INIT</span>: Listening on Port 8080</motion.p>
            <motion.p animate={{ opacity: [0, 1] }} transition={{ delay: 0.8 }}>[09:44:12] <span className="text-blue-400">INGEST_ARXIV</span>: [arXiv:2403.00123] - 100%</motion.p>
            <motion.p animate={{ opacity: [0, 1] }} transition={{ delay: 1.4 }}>[09:44:15] <span className="text-green-400">SYNTHESIS</span>: Extracting high-signal entities...</motion.p>
            <motion.p animate={{ opacity: [0, 1] }} transition={{ repeat: Infinity, repeatDelay: 5 }}>[09:44:18] <span className="text-yellow-400">MATCH</span>: Found 12 GitHub repos for 'KV-Cache'</motion.p>
        </div>
...
const OrbitingIcon = ({ delay = 0, radius = 80, duration = 10, icon: Icon, label }: any) => {
  return (
    <motion.div
      animate={{ rotate: 360 }}
      ...
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        >
          ...
          <p className="relative z-10 text-white/40 max-w-2xl mb-12 leading-relaxed text-sm">
             PulseFeed's inference engine operates at the intersection of retrieval and reasoning, 
             extracting signal from raw technical firehoses to build a persistent context graph.
          </p>

          {/* Animation Area */}
          <div className="relative w-full h-[320px] flex items-center justify-center">
            {/* Trace Lines */}
            {[0, 60, 120, 180, 240, 300].map(r => <TraceLine key={r} rotate={r} />)}
            ...
            {/* Orbiting Icons */}
            <OrbitingIcon icon={Globe} radius={140} duration={18} delay={0} label="ArXiv" />
            <OrbitingIcon icon={Code} radius={180} duration={28} delay={-5} label="GitHub" />
            <OrbitingIcon icon={FileText} radius={160} duration={22} delay={-10} label="Logs" />
            <OrbitingIcon icon={Search} radius={120} duration={14} delay={-2} label="Deep Web" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          >
            ...
            <h4 className="text-xl font-bold text-white mb-4">Zero-Latency Synthesis</h4>
          ...
          >
            ...
            <h4 className="text-xl font-bold text-white mb-4">Hyper-Personalized Filters</h4>
            <p className="text-white/40 text-[13px] leading-relaxed text-center">
