...
const NODES = [
  { id: 1, dx: -260, dy: -120, label: 'arxiv/2401.0412',   color: '#10B981', delay: 0.2 },
  { id: 2, dx:  240, dy: -140, label: 'nature.com/ml',      color: '#60A5FA', delay: 0.7 },
  { id: 3, dx: -280, dy:   60, label: 'github/llm-paper',   color: '#F59E0B', delay: 1.2 },
  { id: 4, dx:  270, dy:   80, label: 'hackernews/top',     color: '#10B981', delay: 1.7 },
  { id: 5, dx:  -90, dy:  190, label: 'ieee/neural-arch',   color: '#60A5FA', delay: 2.2 },
  { id: 6, dx:  100, dy: -210, label: 'openai.com/research',color: '#F59E0B', delay: 2.7 },
]
...
function BrainSVG({ glowing }) {
  ...
  return (
    ...
    >
      ...
      {glowing && (
        <>
          {[[142,100],[258,100],[120,155],[280,155],[160,210],[240,210],[200,75]].map(([cx, cy], i) => (
            <motion.circle
              key={`node-${i}`}
              cx={cx} cy={cy} r={3}
              fill="#10B981"
...
function DataNode({ node }) {
  ...
  return (
    ...
    >
      ...
      <circle cx={cx} cy={cy} r={9} fill="none" stroke={node.color} strokeWidth={0.8} opacity={0.5} />
      {/* Trail line toward brain */}
      <motion.line
        x1={cx} y1={cy}
        x2={cx + node.dx * 0.5} y2={cy + node.dy * 0.5}
        stroke={node.color}
        strokeWidth={0.5}
        strokeOpacity={0.3}
    ...
    >
      ...
      <nav className="absolute top-0 inset-x-0 flex items-center justify-between px-8 py-6 z-10">
        <div className="flex items-center gap-2.5">
          ...
          <span style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 600, fontSize: '1.1rem', letterSpacing: '0.02em' }}>
            PulseBoard
          </span>
        </div>
        <button
      ...
      {/* Brain scene */}
      <div className="relative z-0" style={{ width: 'min(460px, 90vw)', height: 'min(380px, 75vw)', padding: '0 24px' }}>
      ...
      >
        ...
        >
          ...
          <br />
          <em style={{ fontStyle: 'italic', fontWeight: 400 }}>From Noise to Knowledge.</em>
        </h1>
        ...
        >
          An AI Research Agent that filters the world's chaos
          into your personal power source — refreshed every 6 hours.
        </p>
        ...
        <motion.div
          ...
          style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.72rem', color: '#8B7355' }}
        >
          {['10 hrs/week reclaimed', 'Zero FOMO', 'Gemini 2.5 · DuckDuckGo'].map((s, i) => (
        ...
        >
          ...
          <span style={{ fontSize: '0.8rem', color: '#8B7355', fontFamily: "'JetBrains Mono', monospace" }}>
            no cost · no card
          </span>
        </motion.div>
      </motion.div>
    ...
    <div className="relative w-full h-40 flex flex-col justify-center gap-2 overflow-hidden">
      {[1, 0.6, 0.3, 0.7, 0.4, 0.8].map((op, i) => (
        <motion.div
          ...
          animate={{ opacity: [op, op * 0.4, op], x: [0, (i % 2 === 0 ? 3 : -3), 0] }}
...
function PulseVisual() {
  const lines = [
    { w: '85%', bold: true, text: 'AlphaFold 4 predicts protein folding at atomic precision.' },
    { w: '70%', bold: false, text: 'Gemini 2.5 Flash sets new reasoning benchmark.' },
    ...
    >
      ...
      <div className="flex-1">
        <p
          ...
          style={{ color: benefit.accent, fontFamily: "'JetBrains Mono', monospace" }}
        >
          {benefit.tag}
        </p>
        <h2
        ...
        <p className="mt-5 text-sm leading-relaxed text-slate-400" style={{ maxWidth: '38ch' }}>
          {benefit.body}
        </p>
      </div>
...
const TICKER = [
  'Gemini 2.5 Flash', 'React Server Components', 'Data Science Summit · Berlin',
  'AlphaFold 4', 'Rust 2.0 Stable', 'NeurIPS 2026', 'Llama 4 Released',
  'PyTorch 3.0', 'Google I/O 2026', 'EU AI Act Enforcement',
]
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="py-40 px-6 flex flex-col items-center text-center relative overflow-hidden">
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: 'radial-gradient(ellipse 50% 35% at 50% 60%, rgba(16,185,129,0.06) 0%, transparent 100%)',
        }}
      />
      <motion.p
      ...
      >
        Stop scrolling.
        <br />
        <em style={{ fontStyle: 'italic', color: '#10B981' }}>Start pulsing.</em>
      </motion.h2>
      <motion.p
        initial={{ opacity: 0, y: 16 }}
        animate={inView ? { opacity: 1, y: 0 } : {}}
